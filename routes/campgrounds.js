...
router.get("/:id/managefunds",middleware.isLoggedIn,function(req,res){
    ...
    res.render("campgrounds/managefunds",{parm:{id}});

      Campground.findById({ "_id": id },{funds:{remainingFunds}} ,function(err,found){
...
//add Funds
router.post("/managefunds/:id/addfunds",middleware.isLoggedIn,function(req,res){
    var fundToAdd = req.body.fundToAdd;
    var societyId = req.params.id;
    console.log(societyId);
    console.log(fundToAdd);
    
//    Campground.find({funds : { $elemMatch:{"remainingFunds":0}}},function(err,foundFund){
//     console.log("foundFund");
//     console.log(foundFund);

//    });
//    Campground.findByIdAndUpdate({ "_id": societyId },{ $inc: {"remainingFunds":  fundToAdd} },function(err,found){
    
//     if(err) console.log(err);
//    }
   Campground.findByIdAndUpdate({ "_id": societyId },{funds:{remainingFunds:fundToAdd}} ,function(err,found){
    
    if(err) console.log(err);
    console.log(found);
    res.render("/campgrounds/managefunds");
   }
)
...
router.post("/adminpanel/:id/addtreasurertosociety",function(req,res){
 ...
 var found=0;
          ...
          User.findByIdAndUpdate(foundSociety.treasurer,{isTreasurer:false},function(err){
            console.log("Removed Previous Treasuer");
